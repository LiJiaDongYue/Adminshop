...
define(['dialog','valid', 'jForm'], function (dialog) {
    return {
        companyIsActive : function(){
            $('#companyCert-table').on('click','a.is-active',function(){
                var id = $(this).attr('data-id');
                var val = $(this).attr('data-val');
                $.ajax({
                    cache: false,
                    type: "GET",
                    url: '../CompanyCert/active?id=' + id + '&flag=' + val,
                    dataType: "json",
        ...
        validCompanyCertForm : function () {
            $('#company-cert-form').validate({
                errorElement: 'small',
                ...
                submitHandler: function (form) {
                    ...
                    $(form).ajaxSubmit({
                        url: '../CompanyCert/save',
                        type: 'POST',
                        dataType: 'json',
                        success: function (resp) {
                            if(resp && resp.status === 1){
                                history.go(-1);
                               // window.location.href = "#sys/companyCert/&p=1";
                            } else {
