!function($) {
	/*
	 * 基于Bootstrap Typeahead改造而来的自动完成插件 Author：F.L.F Site: http://digdata.me
	 * =================================
	 */
	...
	Autocomplete.prototype = {
		...
		onSelect : function(element,menu) {
		...
		select : function() {
			...
			//通过隐藏的id来判断，否则当没有查询到值时会把输入的值清空
			if(val!=null&&val!='' && realVal!=null&&realVal!='')
			//if(realVal!=null&&realVal!='')
				this.$element.val(this.updater(val)).attr("real-value", realVal).change();	
			
			
			this.onSelect(this.$element,this.$menu);
			return this.hide();
		}
		...
		highlighter : function(item) {
			...
			return item.replace(new RegExp('(' + query + ')', 'ig'), function(
							$1, match) {
						return '<strong style="color:#FF6600;">' + match
								+ '</strong>'
					})
		}
	...
	$.fn.autocomplete = function(option) {
		return this.each(function() {
			var $this = $(this), data = $this.data('autocomplete'), options = typeof option == 'object'
					&& option
			if (!data)
	...
	$(document).on('focus.autocomplete.data-api',
			'[data-provide="autocomplete"]', function(e) {
				var $this = $(this)
